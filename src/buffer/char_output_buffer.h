...
class CharArrayOutputBuffer {
 public:
  CharArrayOutputBuffer(char* output) : output(output) { pos = 0; }
  ...
  void write(const char* content, size_t length) {
    if (pos + length > STRING_OUTPUT_BUFFER_SIZE) {
      ...
      if (length > STRING_OUTPUT_BUFFER_SIZE) {
        strncpy(output, &content[pos], length);
  ...
  void flush() {
    strncpy(buffer, &output[pos], STRING_OUTPUT_BUFFER_SIZE);
    //str->append(buffer, pos);
    pos = 0;
  }

  template <class T>
  CharArrayOutputBuffer& operator<<(const T& t) {
    Serializer<T, CharArrayOutputBuffer>::serialize(t, *this);
    return *this;
  }
